...
#' axiom if the data set is generated by irrational behavior. Following Bronars' (1987) approach, we measure the
#' power using Becker's (1962) notion of irrational behavior. More specifically, we generate irrational behavior by
#' simulating a consumer who makes consumption choices randomly from his or her budget set such that the budget set is
#' exhausted. Bronars power index measures probability of rejecting the null hypothesis that the random consumer is making
#' utility maximizing choices. Essentially, Bronars power index is the number of times a violation is detected in the
...
#'
#' @return This function returns \code{power} which computes the power of a given axiom against uniformly
#' random behavior at a given efficiency level.
#'
#' @export
#'
#' @section References:
...
#' nrow = 10, ncol = 3, byrow = TRUE)
#'
#' # compute Bronars' power for GARP at full efficiency
#' bronars(p,q)
#'
#' # compute Bronars' power for SARP at 99\% efficiency
